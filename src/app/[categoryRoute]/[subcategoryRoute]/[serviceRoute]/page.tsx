...
export async function generateStaticParams() {
  const staticParams: Array<{ categoryRoute: string, subcategoryRoute: string, serviceRoute: string }> = [];

  for (const categoryRoute in serviceRouteStructure) {
    for (const subcategoryRoute in serviceRouteStructure[categoryRoute]) {
      serviceRouteStructure[categoryRoute][subcategoryRoute].forEach(serviceRoute => {
