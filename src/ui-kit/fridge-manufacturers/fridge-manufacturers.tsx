...
export default function FridgeManufacturers({ customClass }: { customClass?: string }) {
  ...
  const fridgeManufacturersJsx = useMemo(() => {
    return fridgeManufacturers.map((params, index) => <React.Fragment key={index}>{fridgeManufacturerFactory({ ...params, styles })}</React.Fragment>);
  }, []);
  ...
  return <section className={composeClassName(styles['fridge-manufacturers'], customClass)}>
    ...
    {!isMobile
      ? <div className={styles['fridge-manufacturers-list']}>{fridgeManufacturersJsx}</div>
      : <Slider
